...
async def async_setup_entry(
    ...
    def _async_add_kick_buttons():
        ...
        for mac, device_info in device_statistics.items():
            ...
            # Create unique identifier for this device button (MAC only for consistency)
...
class DeviceKickButton(CoordinatorEntity[SharedDataUpdateCoordinator], ButtonEntity):
    ...
    def __init__(
        ...
        if device_name == "Unknown" or device_name == "*":
            self._attr_name = f"Disconnetti {self._device_mac}"
        else:
            self._attr_name = f"Disconnetti {device_name}"
        # Changed: Use MAC-only unique ID for consistency
        self._attr_unique_id = f"{DOMAIN}_{unique_id}_kick"
        
        
        # Device info - associate with the connected device (not AP)
